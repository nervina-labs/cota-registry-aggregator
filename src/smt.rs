...
use nft_smt::registry::{CompactNFTRegistryEntriesBuilder, KVPair, KVPairBuilder, KVPairVecBuilder};
...
#[test]
fn create_smt() {
    let lock_hash: [u8; 32] = [234, 40, 201, 143, 56, 180, 165, 122, 168, 23, 86, 177, 103, 187, 55, 250, 66, 218, 246, 126, 219, 201, 134, 58, 251, 129, 114, 9, 110, 211, 1, 194];
    println!("{:?}", hex::encode(&lock_hash));
    let lock_hashes = vec![lock_hash];
    let (root_hash, witness_data) = generate_smt(vec![], lock_hashes);
    assert_eq!(root_hash, "b6719be3614c76a651b86562c1f35f7cc8d4d2a129dfc75759fc82d601ae5670");
...
#[test]
fn update_smt() {
    let origin_lock_hash: [u8; 32] = [234, 40, 201, 143, 56, 180, 165, 122, 168, 23, 86, 177, 103, 187, 55, 250, 66, 218, 246, 126, 219, 201, 134, 58, 251, 129, 114, 9, 110, 211, 1, 194];
    let origin_lock_hashes = vec![origin_lock_hash];

    let lock_hash: [u8; 32] = [50, 247, 246, 45, 198, 8, 45, 245, 35, 87, 121, 193, 186, 170, 5, 86, 60, 208, 149, 227, 148, 193, 36, 253, 2, 16, 17, 42, 160, 219, 182, 130];
    let lock_hashes = vec![lock_hash];

    let (root_hash, witness_data) = generate_smt(origin_lock_hashes, lock_hashes);
    assert_eq!(root_hash, "bae562505da06ad2226ffc876b09d97b908e63820bb49a7f31d88833b34a800a");
